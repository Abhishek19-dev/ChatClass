...
export const joinGroup = (group , user) =>async(dispatch)=>{
 ...
 export const getPublicGroupUser = (id) =>async(dispatch)=>{
    try {
     dispatch({
         type: GET_ALL_PUBLIC_CHAT_USER_REQUEST
      })
      const config = {headers:{"Content-type":"application/json"}}
      const {data} = await axios.post("api/v1/publicGroupChatUser",{id:id},config)
    //   console.log("profiler data",data)
      dispatch({
          type : GET_ALL_PUBLIC_CHAT_USER_SUCCESS,
          payload : data
      })
